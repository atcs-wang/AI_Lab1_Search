...
#TEXT = 'text'
COLORS = {FLOOR : 'pale green', CARPET : 'RoyalBlue1', WALL : 'gray25', 
...
class Roomba_GUI(Search_GUI):
    ...
    def calculate_box_coords(self, r : int, c : int) -> Tuple[int, int, int, int]:
        ...
        y1 = h * r // self.height
        x2 = w * (c + 1) // self.width
        y2 = h * (r + 1) // self.height
        return (x1, y1, x2, y2)

    def calculate_center_coords(self, r : int, c : int) -> Tuple[int, int]:
        w = self.canvas.winfo_width() # Get current width of canvas
        h = self.canvas.winfo_height() # Get current height of canvas
        x = int(w * (c + .5)) // self.width
        y = int(h * (r + .5)) // self.height
        return (x, y)
    ...
    #Override
    def draw_path(self):
        path : Sequence[RoombaState] = self.current_state.get_path()
        ...
        self.canvas.create_oval(*self.calculate_box_coords(*path[0].position), fill= '', outline = COLORS[START], tag= START)
    ...
    def draw_background(self):
        ...
        for r in range(0,self.height):
            for c in range(0,self.width):
                self.canvas.create_rectangle(*self.calculate_box_coords(r,c), fill= COLORS[maze[r][c]], tag='terrain_block')
    ...
    def click_canvas_to_action(self, event) -> Coordinate:
        ...
        cur_r, cur_c = self.current_state.position
        return Coordinate(row - cur_r, col - cur_c)
...
if __name__ == "__main__":
    ...
    gui = Roomba_GUI(initial_state,algorithm_names=ALGORITHMS.keys(), strategy_names=STRATEGIES.keys(), heuristics=ROOMBA_HEURISTICS)
